    ...
    FractletListener
{
  ...
  public
  FractletCanvas(Fractlet t, FractletProperties p)
  {
    master = t;
    properties = p;
  ...
  paintMe(Graphics g, int w, int h, int xoff, int yoff)
  {
      ...
      case Fractlet.MODE_MAIN:
      case Fractlet.MODE_1ST_LEVEL:
      case Fractlet.MODE_2NDARY:
...
*/
      ...
      throws IOException
  {
      ...
      LoadedDef def = l.loadNextTaggedDef(Fractlet.SAVE_FILE_TAG, args, close);
...
//System.err.println("Loading fractal image, saved by Fractlet version: <unknown>");
      ...
      throws IOException
  {
	  ...
	  fig_def = l_defs.findAssignmentValueFor(Fractlet.SAVE_TAG_FIGURE, false, true);

	  if (fig_def == null || !properties.loadFrom(fig_def)) {
	      throw new IOException("No pattern definitions found from the savefile.");
	  }
	  
	  if ((currBgColour = loadColour(fig_def.findAssignmentValueFor(Fractlet.SAVE_TAG_BACKGROUND,
								    false, false))) == null) {
	      currBgColour = DEF_BG_COLOUR;
	  }

	  if ((def = fig_def.findAssignmentValueFor(Fractlet.SAVE_TAG_PALETTE, false, false)) != null
	      && def.containsList()) {
	  ...
	  def = fig_def.findAssignmentValueFor(Fractlet.SAVE_TAG_PATTERN_MAIN, false, true);
	  if (def != null) {
	  ...
	  patterns[PAT_SECONDARY] = null;
	  def = fig_def.findAssignmentValueFor(Fractlet.SAVE_TAG_PATTERN_2NDARY, false, true);
	  if (def != null) {
	      FractalSegment [] tmp = constructPattern(def);
	      if (tmp != null) {
	  ...
	  on_off = false;
	  patterns[PAT_FIRST] = null;
	  def = fig_def.findAssignmentValueFor(Fractlet.SAVE_TAG_PATTERN_1ST_LEVEL, false, true);
	  if (def != null) {
	      FractalSegment [] tmp = constructPattern(def);
	      if (tmp != null) {
	  ...
	  {
	      def = fig_def.findAssignmentValueFor(Fractlet.SAVE_TAG_WINDOW, false, false);
	  ...
	  master.setViewTo(Fractlet.MODE_MAIN);
      }
  ...
  saveFigure(Writer w)
  {
      ...
      out.print("<" + Fractlet.SAVE_FILE_TAG + " VERSION="
      ...
      out.println(Fractlet.SAVE_TAG_FIGURE+" = (");

      // First we'll dump the saveable properties:
      properties.saveTo(out);

      Dimension d = getDrawableSize();
      out.print("\t" + Fractlet.SAVE_TAG_WINDOW + " = (");
      out.print(d.width);
      out.print(", ");
      out.print(d.height);
      out.println(");");
      ...
      out.print("\t"+Fractlet.SAVE_TAG_BACKGROUND + " = (");
      ...
      out.print("\t\t");
      out.println("\t"+Fractlet.SAVE_TAG_PALETTE + " = (");
      out.print("\t\t");
      ...
      synchronized (patterns) {
	  savePatternDef(out, Fractlet.SAVE_TAG_PATTERN_MAIN,patterns[PAT_MAIN]);
	  if (properties.getBooleanValue(FractletProperties.FP_USE_1ST_LEVEL)) {
	      savePatternDef(out, Fractlet.SAVE_TAG_PATTERN_1ST_LEVEL,
			     patterns[PAT_FIRST]);
	  }
	  if (properties.getBooleanValue(FractletProperties.FP_USE_2NDARY)) {
	      savePatternDef(out, Fractlet.SAVE_TAG_PATTERN_2NDARY,
      ...
      out.println(");");
      out.println();
      out.println("</" + Fractlet.SAVE_FILE_TAG + ">");
  ...
  doMenuAction(String id, AWTEvent ev)
  {
	  ...
	  if (second.equals("center")) {
	    ...
	    repaint();
		  ...
		  master.setViewTo(Fractlet.MODE_1ST_LEVEL);
	      } else if (second.equals("to_2ndary")) {
		  master.setViewTo(Fractlet.MODE_2NDARY);
	      } else if (second.equals("to_figure")) {
		  master.setViewTo(Fractlet.MODE_RESULT);
	      } else {
  ...
  public void
  setMenuEnabled(String id, boolean state)
  {
      if (popupComponents == null)
	  return;
